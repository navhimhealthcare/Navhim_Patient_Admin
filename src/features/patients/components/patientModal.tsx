...
import { Patient, PatientFormValues, PatientFormAddValues } from '../types/patient.types'
import {
  EMPTY_FORM, EMPTY_ADD_FORM, formFromPatient,
  validateAddForm, validateUpdateForm, BLOOD_GROUPS,
...
export default function PatientModal({
  open, onClose, onAdd, onEdit, initialData, loading,
}: Props) {
  const isEdit = !!initialData

  // ── Add form state ──
  const [addForm,   setAddForm]   = useState<PatientFormAddValues>(EMPTY_ADD_FORM)
  const [addErrors, setAddErrors] = useState<Partial<Record<keyof PatientFormAddValues, string>>>({})

  // ── Edit form state ──
  const [editForm,   setEditForm]   = useState<PatientFormValues>(EMPTY_FORM)
  const [editErrors, setEditErrors] = useState<Partial<Record<keyof PatientFormValues, string>>>({})
  ...
  // ── Setters ──
  const setA = (k: keyof PatientFormAddValues) =>
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
      setAddForm(p => ({ ...p, [k]: e.target.value }))

  const setE = (k: keyof PatientFormValues) =>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative bg-white rounded-3xl shadow-2xl w-full animate-[fadeUp_0.25s_ease_both] overflow-hidden
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/60 text-[11px]">
                {isEdit ? 'Update full patient profile' : 'Basic info to register patient'}
              </p>
            </div>
          </div>
          ...
          <div className="px-6 py-5 flex flex-col gap-4">
            ...
            <div className="flex items-start gap-2.5 bg-brand-lighter border border-brand-primary/15 rounded-xl px-3 py-2.5">
              ...
              <p className="text-[12px] text-brand-primary font-medium leading-relaxed">
                Patient will be registered with basic details. Additional info like blood group, NFC card, height & weight can be updated after registration.
              </p>
            </div>

            <Field label="Full Name" required error={addErrors.name}>
              <input ref={firstRef} value={addForm.name} onChange={setA('name')}
                placeholder="e.g. Rohit Yadav" className={inputCls} />
            </Field>

            <div className="grid grid-cols-2 gap-3">
              <Field label="Email" required error={addErrors.email}>
                <input type="email" value={addForm.email} onChange={setA('email')}
                  placeholder="e.g. rohit@navhim.org" className={inputCls} />
              </Field>
              <Field label="Phone" required error={addErrors.phone}>
                <input type="tel" value={addForm.phone} onChange={setA('phone')}
                  placeholder="e.g. 9650224344" className={inputCls} />
              </Field>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <Field label="Date of Birth" required error={addErrors.dob}>
                <input type="date" value={addForm.dob} onChange={setA('dob')} className={inputCls} />
              </Field>
              <Field label="Gender" required error={addErrors.gender}>
                <select value={addForm.gender} onChange={setA('gender')} className={inputCls}>
          ...
          <>
            <div className="px-6 py-5 flex flex-col gap-4 max-h-[70vh] overflow-y-auto scrollbar-hide">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 border-2 border-dashed border-brand-primary/20 bg-surface flex items-center justify-center">
                    {avatarPreview ? (
                      ...
                      <span className="text-2xl">{genderIcon(editForm.gender)}</span>
                    )}
                  </div>
                  <label className="flex-1 cursor-pointer">
                    <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-dashed border-brand-primary/30 hover:border-brand-primary hover:bg-brand-lighter transition-all">
                      ...
                      <span className="text-[12.5px] font-semibold text-gray-500 truncate">
                        {editForm.avatar ? editForm.avatar.name : editForm.existingAvatar ? 'Change photo' : 'Upload photo (JPG, PNG, WEBP)'}
                      </span>
                    </div>
                    <input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only"
                      onChange={e => setEditForm(p => ({ ...p, avatar: e.target.files?.[0] ?? null }))} />
                  </label>
                  {editForm.avatar && (
                    <button type="button" onClick={() => setEditForm(p => ({ ...p, avatar: null }))}
                  ...
                  {!editForm.avatar && editForm.existingAvatar && (
                    <button type="button" onClick={() => setEditForm(p => ({ ...p, existingAvatar: '' }))}
                      className="w-8 h-8 rounded-lg bg-danger-bg text-danger flex items-center justify-center text-sm hover:bg-red-100 transition-colors flex-shrink-0">
                      🗑
                    </button>
                  )}
              ...
              <Field label="Full Name" required error={editErrors.name}>
                <input ref={firstRef} value={editForm.name} onChange={setE('name')}
                  placeholder="e.g. Rohit Yadav" className={inputCls} />
              </Field>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Phone" required error={editErrors.phone}>
                  <input type="tel" value={editForm.phone} onChange={setE('phone')}
                    placeholder="e.g. 9650224344" className={inputCls} />
                </Field>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Date of Birth" required error={editErrors.dob}>
                  <input type="date" value={editForm.dob} onChange={setE('dob')} className={inputCls} />
                </Field>
                <Field label="Gender" required error={editErrors.gender}>
                  <select value={editForm.gender} onChange={setE('gender')} className={inputCls}>
                    <option value="">Select gender</option>
                    <option value="male">Male</option>
                    <option value="female">Female</option>
                    <option value="other">Other</option>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Blood Group">
                  <select value={editForm.bloodGroup} onChange={setE('bloodGroup')} className={inputCls}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Height (ft)">
                  <input type="number" value={editForm.height} onChange={setE('height')}
                    placeholder="e.g. 6" className={inputCls} min={0} step="0.1" />
                </Field>
                <Field label="Weight (kg)">
                  <input type="number" value={editForm.weight} onChange={setE('weight')}
                    placeholder="e.g. 70" className={inputCls} min={0} />
                </Field>
              </div>

              {/* Emergency Contact */}
              <Field label="Emergency Contact">
                <input type="tel" value={editForm.emergencyContact} onChange={setE('emergencyContact')}
                  placeholder="e.g. 9870306705" className={inputCls} />
              </Field>

              {/* Status Toggle */}
              <div className="flex items-center justify-between p-4 bg-surface rounded-2xl border border-brand-primary/10">
                <div>
                  <p className="text-[13px] font-bold text-navy">Account Status</p>
                  <p className="text-[11px] text-gray-400">Enable or disable patient account access</p>
                </div>
                <button
                  type="button"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Field label="" error={editErrors.lat}>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-300 pointer-events-none">LAT</span>
                      <input type="number" step="any" value={editForm.lat} onChange={setE('lat')}
                        placeholder="e.g. 28.5600" className={`${inputCls} pl-9`} />
                    </div>
                  </Field>
                  <Field label="" error={editErrors.lng}>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-300 pointer-events-none">LNG</span>
                      <input type="number" step="any" value={editForm.lng} onChange={setE('lng')}
                        placeholder="e.g. 77.2116" className={`${inputCls} pl-9`} />
                    </div>
                  </Field>
                </div>
                {editForm.lat && editForm.lng && (
                  <a href={`https://www.google.com/maps?q=${editForm.lat},${editForm.lng}`}
