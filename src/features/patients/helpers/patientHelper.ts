...
export const formFromPatient = (p: Patient): PatientFormValues => ({
  ...
  lat:
    p.location?.coordinates?.[1] != null
      ? String(p.location.coordinates[1])
      : "",
  lng:
    p.location?.coordinates?.[0] != null
      ? String(p.location.coordinates[0])
      : "",
  isActive: p.isActive !== false,
});

export const isOnlyStatusChanged = (
  form: PatientFormValues,
  original: Patient,
): boolean => {
  const origForm = formFromPatient(original);

  // Check if isActive is different
  const statusChanged = form.isActive !== origForm.isActive;
  if (!statusChanged) return false;

  // Check if any other field is different
  const otherFields: (keyof PatientFormValues)[] = [
    "name",
    "email",
    "phone",
    "dob",
  ...
  const anyOtherChanged = otherFields.some((key) => {
    if (key === "avatar") return !!form.avatar; // If new avatar uploaded, it's a change
    return String(form[key] ?? "") !== String(origForm[key] ?? "");
  });

  return !anyOtherChanged;
};

// ── Add payload — only 5 fields ──────────────────────────────────────
export const buildAddPatientPayload = (
...
export const buildUpdatePatientPayload = (
  form: PatientFormValues,
): FormData => {
...
export const filterPatients = (
  ...
  patients.filter((p) => {
    ...
    if (f.bloodGroup && f.bloodGroup !== "" && p.bloodGroup !== f.bloodGroup)
      return false;
