...
import { PatientReport, ReportFilter, CreateReportPayload } from '../types/report.types'
import { fetchPatientReportsAPI, createPatientReportAPI }   from '../services/reportService'
...
export const usePatientReports = (patientId: string) => {
  ...
  const fetchReports = useCallback(async (f: ReportFilter) => {
    if (!patientId) return
    setLoading(true)
    try {
      const params: { patientId: string; fromDate?: string; toDate?: string } = { patientId }
  ...
  useEffect(() => { fetchReports(filter) }, [patientId, filter.fromDate, filter.toDate])
  ...
  const createReport = async (payload: CreateReportPayload): Promise<boolean> => {
    ...
    try {
      ...
      showToast.error(err?.response?.data?.message || 'Failed to create report.')
      return false
    } finally {
