    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-4">
        <div className="flex gap-1.5">
          {[1, 2, 3, 4, 5].map((star) => {
            ...
            const isHalf =
              displayRating > star - 1 && displayRating < star && displayRating % 1 !== 0;
            ...
            return (
              ...
              >
                {/* Background star */}
                <div className="text-4xl text-gray-200 group-hover:text-gray-300 transition-colors">
                  ★
                </div>

                {/* Filled star overlay */}
                <div
                  className="absolute top-0 left-0 text-4xl text-yellow-400 transition-all overflow-hidden group-hover:text-yellow-300"
                  style={{
                    width:
                      isFilled
                        ? "100%"
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <div className="text-[11px] text-gray-400 font-medium">
            {ratingOptions.find((r) => r.value === displayRating)?.label || ""}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {ratingOptions.map((option) => {
          const isSelected = ratingValue === option.value;
          return (
            <button
              key={option.value}
              type="button"
            ...
            >
              {option.value === 0 ? "None" : option.value.toFixed(1)}
            </button>
          );
        })}
      </div>
        ...
        <div className="px-6 py-5 flex flex-col gap-4 max-h-[70vh] overflow-y-auto scrollbar-hide">
          ...
          <Field label="Profile Image">
            <div className="flex items-center gap-3">
              {/* Preview — new file takes priority, then existing URL */}
              <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 border-2 border-dashed border-brand-primary/20 bg-surface flex items-center justify-center">
                {form.profileImage && form.profileImage instanceof Blob ? (
                  <img
                    src={URL.createObjectURL(form.profileImage)}
                    alt="new preview"
                    className="w-full h-full object-cover"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Consultation Fee (₹)" required={!form.isFree} error={errors.consultationFee}>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-bold">
                  ₹
                </span>
